...
game_month  =    7      # month of the game
timezone    =    0      # offset from UTC ( CST = -6) (Google seems to have that covered)

# game settings
first_penalty = 3       # points for the first hit
nnmax_vid     = 3       # maximum number of videos in play
...
async def reset_cmd():
    ...
    msg = await chans[iichan_cmd].send( \
        ...
        " \n" +\
        "<:mc_album:686011859207585817> - report a hit on self\n" +\
        "<:mc_divacup:685130640450256915> - report a hit on multiple people\n" +\
        "\U0001F4CA - progress report\n" +\
        "\U0001F389 - credit\n" + \
        "\U0001F4E5 - join game\n" +\
        "\U0001F4E4 - withdraw from game\n" +\
        "==========================")
    await msg.add_reaction("<:mc_album:686011859207585817>")
    await msg.add_reaction("<:mc_divacup:685130640450256915>")
    await msg.add_reaction("\U0001F4CA")
    await msg.add_reaction("\U0001F389")
    await msg.add_reaction("\U0001F4E5")
    await msg.add_reaction("\U0001F4E4")
    return 1
...
async def reset_gam():
    ...
    msg = await chans[iichan_gam].send( \
        ...
        "\n" +\
        "\U0001F955 - carrot in a box\n" +\
        "\U0001F4A1 - random trivia\n" +\
        "============================")
    await msg.add_reaction("\U0001F955")
    await msg.add_reaction("\U0001F4A1")
    return 1
...
async def reset_inf():
    ...
    await chans[iichan_inf].send(\
        ...
        "#announcements - announcement related to the game, e.g. game start, bonus period, special events :speaker:\n" +\
        "#point-changes - any point changes in the game, whether it be from natural hit or videos\n" +\
        "#commands - where players can use command :mute:\n" +\
        "#risky-click - the risky click game\n" +\
        "#voting - for when there are things to be voted on:speaker:\n" +\
        "#mini-games - for short games to play:mute:\n" +\
        "\n" +\
        "__Legend__\n" +\
        ":mute:recommend to mute it\n" +\
        ":speaker:recommend to not mute it\n" +\
        "The rest of the channels can be muted by preference")
    ...
    await chans[iichan_inf].send(\
        ...
        "\n" +\
        "Every time you hear the song, you get a hit, and you report it in the #commands channel.\n" +\
        "\n" +\
        ...
        "Document: <https://docs.google.com/document/d/155IahiH3bUtuyeviKQPhilocg8NqzyZMcRypMl9Ks64/edit?usp=sharing>")
        
    await chans[iichan_inf].send(\
        "==========================\n" +\
        "__**Risky Click Game**__\n" +\
        "Every day, a random link will be posted in the #risky-click channel, which would lead you to a video that may or may not have the song. If you would like to risk it, open the link, and react to the message containing the link with :white_check_mark: . If the video did not contain the song, you lose a point; otherwise, you gain a point.\n" +\
        "\n" +\
        "At the time of resolution, the bot will automatically change the points, so don't report this!")
...
async def load_game():    
    ...
    for ii in np.arange(len(t_bounds)):
        ...
        print(str(ii) + ": " + get_time_string(t_bounds[ii]))
...
async def report_self(discord_id):
    ...
    if ( ind == -1 ):
        ...
        await chans[iichan_cmd].send("Beep Bop. Comment within the next 30 seconds if you would like to do so, or type 'x' to skip!")
        ...
        try:
            ...
            msg = await client.wait_for('message', timeout=30.0, check=same_user)
            ...
            await chans[iichan_cmd].send("Beep bop. Reported " + players[ind].name + " for " + str(p) + " point! \nThe channel will reset by itself in 5 second...")
            await chans[iichan_pnt].send("Beep bop. Reported " + players[ind].name + " for " + str(p) + " point!")
            time.sleep(5.0)
            await reset_cmd()
            return
...
async def report_group(discord_id):
    ...
    for ii in np.arange(len(members)):
        ...
        if ( ind == -1 ):
            await chans[iichan_cmd].send("Beep. I can't find player "+members[ii].name+". Are they in the game?")
            return
        
        if ( not players[ind].is_in_game ):
    ...
    await chans[iichan_cmd].send(sstr)
    await chans[iichan_pnt].send(sstr)
...
async def join_game(user):
    ...
    if alreadyInGame:
        ...
        if ( tnow < t_bounds[1] ):
            p = Player(user.nick,user.id,tnow,0, True)
        else:
            if len(players) == 0:
                p = Player(user.nick,user.id,tnow,0, True)
            else:
...
async def cue_reset_cmd():
    msg = await chans[iichan_cmd].send("Hit \U0001F504 when you're done!")
...
async def gam_carrot():
    ...
    try:
        ...
        if ( (box_switched and bot_has_carrot) or (not box_switched and not bot_has_carrot) ):
            ...
            pl = players[find_player(user.id)]
...
async def init_link():
    ...
    if debugMode == 0:
        t_resolve = np.ceil((t_issue-43200)/86400)*86400+43200
    elif debugMode == 1:
    ...
    msg = await chans[iichan_rsk].send( \
            ...
            "<" + url + ">\n\n" + \
            "Time to resolve: " + get_time_string(t_resolve) + "\n\n" + \
...
async def cue_reset_gam():
    msg = await chans[iichan_gam].send("Hit \U0001F504 when you're done!")
    await msg.add_reaction("\U0001F504")
...
async def phase_warn_of_start():
    await chans[iichan_ann].send("Beep beep. Game starts in 24 hours!", file=discord.File('pics/warn.png'))
...
async def phase_game_end():
    ...
    await chans[iichan_ann].send(\
        ...
        "*Mini game losers*: " + strMmin[3:] + " ("+str(mmax)+")\n" +\
        "*Mini game winners*: " + strMmax[3:] + " ("+str(mmin)+")\n")
...
async def on_message(msg): 
    ...
    if msg.channel == chans[iichan_sha]:
        if msg.content.lower() == 'list commands':
            await chans[iichan_sha].send(\
                "list player, " +\
                "list player ext, " +\
                "list hit, " +\
...
async def on_reaction_add(rxn, user):
    ...
    if rxn.message.channel == chans[iichan_cmd]:
        ...
        if isinstance(rxn.emoji, str):
            if ( rxn.emoji == '\U0001F4CA' ):
                print('report progress')
                await get_progress()
                
            elif ( rxn.emoji == "\U0001F389" ):
                await get_credit()
                
            elif ( rxn.emoji == '\U0001F504' ):
                await reset_cmd()
                
            elif ( rxn.emoji == '\U0001F4E5' ):
                await join_game(user)
                
            elif ( rxn.emoji == '\U0001F4E4' ):
    ...
    if rxn.message.channel == chans[iichan_gam]:
        if isinstance(rxn.emoji, str):
            if ( rxn.emoji == '\U0001F955' ):
                await gam_carrot()
            
            elif rxn.emoji == '\U0001F4A1':
                await give_trivia()
