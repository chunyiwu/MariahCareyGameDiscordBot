...
class Link:
    ...
    async def resolve(self):
        ...
        if len(ps) != 0 :
            ...
            await chans[iichan_pnt].send(sstr, file=discord.File(img_pnt[pt+2]))
...
async def report_group(discord_id):
    ...
    await chans[iichan_pnt].send(sstr, file=discord.File(img_pnt[p+2]))
...
async def init_link():
    ...
    msg = await chans[iichan_rsk].send( \
            ...
            "\U0001F44C means this link is resolved, and the video is safe.\n"+\
             "<:mc_album:686011859207585817> means this link is resolved, and the video is a trap."   )
...
async def on_ready():
    ...
    print(np.mod(time.mktime(time.strptime('2020/08/08 20:00:00',"%Y/%m/%d %H:%M:%S")),86400))
    await init_link()
    ...
    if rxn.message.channel == chans[iichan_cmd]:
        ...
        if isinstance(rxn.emoji, str):
            if ( rxn.emoji == '\U0001F4CA' ):
                await chans[iichan_sha].send(user.nick+"; progress")
                await get_progress()
                
            elif ( rxn.emoji == "\U0001F389" ):
                await chans[iichan_sha].send(user.nick+"; credit")
                await get_credit()
                
            elif ( rxn.emoji == '\U0001F504' ):
                await chans[iichan_sha].send(user.nick+"; reset command")
                await reset_cmd()
                
            elif ( rxn.emoji == '\U0001F4E5' ):
                await chans[iichan_sha].send(user.nick+"; join")
                await join_game(user)
                
            elif ( rxn.emoji == '\U0001F4E4' ):
                await chans[iichan_sha].send(user.nick+"; quit")
            ...
            if ( rxn.emoji.name == 'mc_album' ):
                await chans[iichan_sha].send(user.nick+"; report self")
                await report_self(user.id)
                
            elif ( rxn.emoji.name == 'mc_divacup' ):
                await chans[iichan_sha].send(user.nick+"; report multiple")
    ...
    if rxn.message.channel == chans[iichan_gam]:
        if isinstance(rxn.emoji, str):
            if ( rxn.emoji == '\U0001F955' ):
                await chans[iichan_sha].send(user.nick+"; carrot")
                await gam_carrot()
            
            elif rxn.emoji == '\U0001F4A1':
                await chans[iichan_sha].send(user.nick+"; trivia")
                await give_trivia()
            
                
            elif ( rxn.emoji == '\U0001F504' ):
                await chans[iichan_sha].send(user.nick+"; reset game")
                await reset_gam()
